...
adaptive layer, not split conformal, is what holds it, at both tiers once its
step is stated in the interval's units rather than the series'. And forecast skill is
...
\begin{enumerate}
  ...
  \item Evidence that conformal coverage degrades with aggregation level, from
        ...
        together with the measurement, at building and at city scale, showing
        that the adaptive layer and not split conformal is what holds the level
        --- and that its one parameter transfers between tiers as a fraction of
        the interval width, not as a number (Section~\ref{sec:calibration}).
...
$0.895$. The $0.762$ of Table~\ref{tab:calibration} is the benchmark row under
the step the building used, and it is a units artefact as much as a monsoon
one.
...
than the one with the theorem that holds it, at the city as at the building
once its step is carried over in the interval's units; and forecast skill and available
