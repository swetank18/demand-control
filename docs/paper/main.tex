...
to promise, with no window's day-block bootstrap lower bound under five times,
and not a constant, since one meter returns $0.39$, $0.64$ and $0.42$ in three
successive Junes while its per-step rate does not move.
...
\begin{enumerate}
  \item A measured bracket on horizon risk for a demand ceiling on held-out
        ...
        $0.87$--$0.99$, union bound vacuous on every row. The common formulation is under-protected by a factor of eight to
